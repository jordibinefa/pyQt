...
def hexCRC(st):
    ...
    for ch in st:
        crc = calcByte( ch, crc)
    retCRC = (crc >> 8) | (crc & 0xFF)<<8
    return retCRC

def readModbusAddingCRC_B(st):
    for i in range(0, len(st)):
        print "{:02X}".format(ord(st[i])),
    print "{:04X}".format(hexCRC(st))

def readModbusAddingCRC(st):
    crc = INITIAL_MODBUS
    for ch in st:
        crc = calcByte( ch, crc)
    for i in range(0, len(st)):
        print "{:02X}".format(ord(st[i])),
    print "{:02X}".format(crc & 0xFF),
    print "{:02X}".format(crc >> 8)

def checkModbusWithCrc(st):
	return not hexCRC(st)
